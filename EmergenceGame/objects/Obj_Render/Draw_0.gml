	...
	for(var tX = 0; tX < MAP_W; tX++){
		for(var tY = 0; tY < MAP_H; tY++){
			...
			if(curGrid == global.Interactable_Grid && tX == floor(global.playerX) && tY == floor(global.playerY)){
				...
				draw_sprite_ext(global.hat, i, camera_get_view_x(view_camera[0]) + CAMERA_W/2, camera_get_view_y(view_camera[0]) + CAMERA_H/2, d, 1, 0, c_white, 1);
				if (room != Room_Fishing){
					draw_sprite_ext(Spr_Bow, 0, camera_get_view_x(view_camera[0]) + CAMERA_W/2, camera_get_view_y(view_camera[0]) + CAMERA_H/2, d, 1, 0, c_white, 1);
				}
